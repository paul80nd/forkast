...
// Plan-page tool body: a lightweight "ingredients I want to cook down" list. Each entry shows
// whether the current week already uses it (so the rest are "unused"), and "Suggest recipes" ranks
// the collection by how many of those unused ingredients each recipe uses — add straight to the
// week. Rendered inside the Plan assistant toolbar, which owns the show/hide toggle.
  ...
  return (
    <div className="mt-3 border-t border-line pt-3">
      <p className="text-xs text-muted">
        List ingredients you want to cook down. We’ll flag the ones your week doesn’t already use and
        suggest recipes that do.
      </p>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          {unusedCount === 0 && (
            <span className="text-xs text-muted">
              Your week already uses everything on your list.
            </span>
          )}
        </div>
      )}
      ...
      {results !== null && (
        <div className="mt-3">
          {results.length === 0 ? (
            <p className="text-sm text-muted">No recipes found that use those ingredients.</p>
          ) : (
