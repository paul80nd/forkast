...
2. **★-driven cleanup — built 2026-06-29.** Bulk-delete recipes scored 1–2★ ("bin it" /
   ...
   - The same delete path is also reachable **directly** outside Refine: **tick recipes on
     Browse** (a checkbox per card) and **"Delete selected"** bulk-deletes them, or use the
     **"Delete recipe"** item in the recipe page's "Add to week" split-button dropdown (both
     with confirm) — for clearing things out without ★-rating first.
